...
async function loadTable () {
    ...
    try {
        const response = await fetch('/upddelemployeesavailability', options);
        if (response.ok) {
            const result = await response.json();
            document.getElementById('selectEmployee').value = result.body.employee;
            ...
            document.getElementById('descriptionEmployeeAvailability').value = result.body.description;
        } else {
...
document.getElementById('js-update-record').addEventListener('click', async () => {
    ...
    let description = document.getElementById('descriptionEmployeeAvailability').value;
    ...
    if(!employee ||!reason || !startDate || !endDate || !description) {
    ...
    const options = {
        ...
        body: JSON.stringify({register, executionPath, employee, reason, startDate, endDate, description})
    };
